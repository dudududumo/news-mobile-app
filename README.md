...
### 内容详情页
- **完整内容展示**：标题、正文、图片等内容的完整展示
- **用户信息**：显示作者头像、昵称、发布时间
- **互动功能**：点赞、评论、查看阅读数等互动功能
- **AI标签**：展示基于文章内容生成的AI标签，支持点击跳转创作
- **评论系统**：支持查看评论、发表评论、删除评论（仅作者）
- **图片查看器**：支持多图浏览和大图查看
- **阅读体验优化**：舒适的排版和阅读体验
...
| **数据初始化** | 组件挂载时自动初始化数据，调用 `handleRefresh` 加载第一页数据，并恢复保存的滚动位置 | GET `/api/posts?page=1&limit=10&t={timestamp}` |
...
| **图片网格展示** | 使用 `ImageGrid` 组件展示文章图片；支持1-9张图片的自适应布局；与Feed流保持一致的图片展示效果 | 无特定API，前端组件实现 |
...
| **点赞功能与状态同步** | 实现 `handleLike` 函数支持点赞/取消点赞；使用乐观更新提升用户体验；结合用户ID绑定的本地缓存机制，在Feed流和详情页间同步点赞状态 | POST `/api/posts/{id}/like` 和 `/api/posts/{id}/unlike` |
...
| **用户信息展示** | 显示作者头像、昵称和发布时间；支持编辑时间显示；使用 `dayjs` 库格式化时间；未登录用户显示默认头像和昵称 | 无特定API，用户信息从文章数据获取 |
| **阅读体验优化** | 实现舒适的排版和阅读体验；自定义导航栏样式；支持返回上一页功能；使用品牌主题色统一视觉风格；实现阅读数统计展示 | 无特定API，前端样式和组件实现 |
| **登录状态交互控制** | 未登录用户点击互动功能时显示 `ActionSheet` 提示框；支持直接跳转到登录页面并保留当前页面引用；根据登录状态控制点赞和评论功能的可用性 | 无特定API，调用 `getToken()` 获取认证状态 |
| **滚动到评论区** | 支持通过URL状态参数直接滚动到评论区；评论按钮点击后平滑滚动到评论输入区域；使用 `useRef` 实现精确定位 | 无特定API，前端DOM操作实现 |
